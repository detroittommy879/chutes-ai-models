document.addEventListener("DOMContentLoaded", function () {
  ...
  function downloadCSV() {
    const sortedModels = sortModelsData(
      allModels,
      currentSort.column,
    ...
    sortedModels.forEach((model) => {
      ...
      const inputPrice =
        model.current_estimated_price?.per_million_tokens?.input?.usd || 0;
      const outputPrice =
        model.current_estimated_price?.per_million_tokens?.output?.usd || 0;
      const invocations = model.invocation_count || 0;
      const hasActiveInstance = (model.instances || []).some(
        (inst) => inst.active && inst.verified
      );
  ...
  function downloadMarkdown() {
    ...
    sortedModels.forEach((model) => {
      ...
      const chuteId = model.chute_id || "";
      const modelLink = chuteId
        ? `[${name}](https://chutes.ai/app/chute/${chuteId})`
        : name;
  ...
  function createModalityBadgesHTML(inputs, outputs) {
    const inputBadges = inputs
      .map(
        (m) => `<span class="modality-badge ${getModalityClass(m)}">${m}</span>`
      )
      .join(" ");
    const outputBadges = outputs
      .map(
        (m) => `<span class="modality-badge ${getModalityClass(m)}">${m}</span>`
      )
      .join(" ");
  ...
  async function fetchV1Models() {
    try {
      const response = await fetch(
        `${BASE_PATH}api/models/v1/all`.replace(/\/+/g, "/")
      );
      if (response.ok) {
    ...
    tableHeader.innerHTML = `
            <div class="header-cell" data-column="name">
                ...
                <span class="sort-indicator ${
                  currentSort.column === "name" ? "active" : ""
                }">${getSortIcon("name")}</span>
            </div>
            <div class="header-cell" data-column="template">
                <span>Template</span>
                <span class="sort-indicator ${
                  currentSort.column === "template" ? "active" : ""
                }">${getSortIcon("template")}</span>
            </div>
            <div class="header-cell" data-column="input_modalities">
                <span>Input Modalities</span>
                <span class="sort-indicator ${
                  currentSort.column === "input_modalities" ? "active" : ""
                }">${getSortIcon("input_modalities")}</span>
            </div>
            <div class="header-cell" data-column="output_modalities">
                <span>Output Modalities</span>
                <span class="sort-indicator ${
                  currentSort.column === "output_modalities" ? "active" : ""
                }">${getSortIcon("output_modalities")}</span>
            </div>
            <div class="header-cell" data-column="input_context">
                <span>Input Ctx</span>
                <span class="sort-indicator ${
                  currentSort.column === "input_context" ? "active" : ""
                }">${getSortIcon("input_context")}</span>
            </div>
            <div class="header-cell" data-column="output_context">
                <span>Output Ctx</span>
                <span class="sort-indicator ${
                  currentSort.column === "output_context" ? "active" : ""
                }">${getSortIcon("output_context")}</span>
            </div>
            <div class="header-cell" data-column="visibility">
                <span>Visibility</span>
                <span class="sort-indicator ${
                  currentSort.column === "visibility" ? "active" : ""
                }">${getSortIcon("visibility")}</span>
            </div>
            <div class="header-cell" data-column="sampling">
                <span>Features</span>
                <span class="sort-indicator ${
                  currentSort.column === "sampling" ? "active" : ""
                }">${getSortIcon("sampling")}</span>
            </div>
            <div class="header-cell" data-column="quantization">
                <span>Quant</span>
                <span class="sort-indicator ${
                  currentSort.column === "quantization" ? "active" : ""
                }">${getSortIcon("quantization")}</span>
            </div>
            <div class="header-cell" data-column="created">
                <span>Created</span>
                <span class="sort-indicator ${
                  currentSort.column === "created" ? "active" : ""
                }">${getSortIcon("created")}</span>
            </div>
            <div class="header-cell" data-column="inputPrice">
                <span>Input Price</span>
                <span class="sort-indicator ${
                  currentSort.column === "inputPrice" ? "active" : ""
                }">${getSortIcon("inputPrice")}</span>
            </div>
            <div class="header-cell" data-column="outputPrice">
                <span>Output Price</span>
                <span class="sort-indicator ${
                  currentSort.column === "outputPrice" ? "active" : ""
                }">${getSortIcon("outputPrice")}</span>
            </div>
            <div class="header-cell" data-column="invocations">
                <span>Invocations</span>
                <span class="sort-indicator ${
                  currentSort.column === "invocations" ? "active" : ""
                }">${getSortIcon("invocations")}</span>
            </div>
            <div class="header-cell" data-column="status">
                <span>Status</span>
                <span class="sort-indicator ${
                  currentSort.column === "status" ? "active" : ""
                }">${getSortIcon("status")}</span>
            </div>
            <div class="header-cell" data-column="hot">
                <span>Hot</span>
                <span class="sort-indicator ${
                  currentSort.column === "hot" ? "active" : ""
                }">${getSortIcon("hot")}</span>
            </div>
        `;
  ...
  function createModelRow(model, index) {
    ...
    const createdDate = v1Model?.created
      ? new Date(v1Model.created * 1000).toLocaleDateString()
      : null;
    ...
    const modalityDisplay = createModalityBadgesHTML(
      inputModalities,
      outputModalities
    );
    ...
    const gpuDisplay =
      gpuCount > 0
        ? `<button class="gpu-toggle" onclick="toggleGpu('${gpuId}')">${gpuCount} GPU${
            gpuCount !== 1 ? "s" : ""
          } ▼</button>
               <div id="${gpuId}" class="gpu-details hidden">
                   ${gpus
                     .map(
                       (gpu) =>
                         `<span class="gpu-tag">${gpu.toUpperCase()}</span>`
                     )
                     .join("")}
               </div>`
    ...
    row.innerHTML = `
            <div class="model-cell model-name" title="${escapeHtml(name)}">
                ${
                  chuteId
                    ? `<a href="${modelUrl}" target="_blank" rel="noopener noreferrer" class="model-link">${escapeHtml(
                        name
                      )}</a>`
                    : escapeHtml(name)
                }
                ...
                <small>${
                  inputModalities.length
                    ? inputModalities
                        .map(
                          (m) =>
                            `<span class="modality-badge ${getModalityClass(
                              m
                            )}">${m}</span>`
                        )
                        .join(" ")
            ...
            <div class="model-cell input-context" title="${
              inputContext !== null ? formatNumber(inputContext) : "N/A"
            }">
                ${getVerificationBadge(name, inputContext)}
                ${
                  inputContext !== null
                    ? `<span>${formatNumber(inputContext)}</span>`
                    : '<span style="color: #999;">N/A</span>'
                }
            </div>
            <div class="model-cell output-context" title="${
              outputContext !== null ? formatNumber(outputContext) : "N/A"
            }">
                ${
                  outputContext !== null
                    ? `<span>${formatNumber(outputContext)}</span>`
                    : '<span style="color: #999;">N/A</span>'
                }
            </div>
            ...
            <div class="model-cell features">
                ${
                  supportedFeatures.length > 0
                    ? `<div class="features-mini">${supportedFeatures
                        .map(
                          (f) =>
                            `<span class="feature-mini">${formatFeatureName(
                              f
                            )}</span>`
                        )
                        .join("")}</div>`
                ...
                ${
                  Array.isArray(v1Model?.supported_sampling_parameters)
                    ? `<span class="sampling-badge" title="${v1Model.supported_sampling_parameters.join(
                        ", "
                      )}">${
                        v1Model.supported_sampling_parameters.length
                      } Sampling Params</span>`
                    : ""
                }
            </div>
            <div class="model-cell quantization" title="${
              quantization || "N/A"
            }">
                ${
                  quantization
                    ? `<span class="quant-badge">${escapeHtml(
                        quantization
                      )}</span>`
                    : '<span style="color: #999;">N/A</span>'
                }
            ...
            <div class="model-cell price" title="$${inputPrice.toFixed(
              3
            )}/M tokens">$${inputPrice.toFixed(3)}</div>
            <div class="model-cell price" title="$${outputPrice.toFixed(
              3
            )}/M tokens">$${outputPrice.toFixed(3)}</div>
            <div class="model-cell invocations" title="${invocations.toLocaleString()} invocations">${formatInvocations(
      invocations
    )}</div>
            ...
            <div class="model-cell">
                ${
                  isHot
                    ? '<div class="status-indicator"><span class="status-dot hot"></span><span>Hot</span></div>'
                    : '<span style="color: #999;">Cold</span>'
                }
            </div>
        `;
  ...
  function sortModelsData(models, column, direction) {
    ...
    const sorted = [...models].sort((a, b) => {
      ...
      switch (column) {
        ...
        case "outputPrice":
          aVal =
            a.current_estimated_price?.per_million_tokens?.output?.usd || 0;
          bVal =
            b.current_estimated_price?.per_million_tokens?.output?.usd || 0;
          return direction === "asc" ? aVal - bVal : bVal - aVal;
        ...
        case "context":
          ...
          if (aVal.input !== bVal.input) {
            return direction === "asc"
              ? aVal.input - bVal.input
              : bVal.input - aVal.input;
          } else {
            return direction === "asc"
              ? aVal.output - bVal.output
              : bVal.output - aVal.output;
          }
        ...
        case "status":
          aVal = (a.instances || []).some(
            (inst) => inst.active && inst.verified
          )
            ? 1
            : 0;
          bVal = (b.instances || []).some(
            (inst) => inst.active && inst.verified
          )
            ? 1
            : 0;
        ...
        case "sampling":
          try {
            const getSamplingCount = (model) => {
              const v1Model = v1ModelsMap.get(model.name);
              return Array.isArray(v1Model?.supported_sampling_parameters)
                ? v1Model.supported_sampling_parameters.length
                : 0;
            };
  ...
  function updateStats(data) {
    ...
    document.getElementById("showingModels").textContent =
      data.items?.length || 0;
...
Context: ${formatNumber(testResult.inputContext || inputContext)}`;
    ...
    // Show date/time inline next to the verified text and keep the tooltip for details.
    // Keep the checkmark and reduce the timestamp font with CSS (.verification-time)
    const inlineTime = `${dateStr} ${timeStr}`;
    const safeInlineTime = escapeHtml(inlineTime);

    return `<span class="verification-badge" title="${tooltipText}" aria-label="Verified on ${safeInlineTime}">✓ Verified <span class="verification-time">${safeInlineTime}</span></span>`;
  }
  ...
  async function loadTokenTestResults() {
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          "✅ Loaded token test results for",
          Object.keys(tokenTestResults).length,
          "models"
        );
      }
    } catch (error) {
  ...
  runLatencyTestBtn.addEventListener("click", async () => {
    ...
    const testPrompt =
      testPromptInput.value.trim() || "Write a haiku about programming.";
    ...
    const modelsToTest = Array.from(selectedForTest)
      ...
      .filter(
        (m) =>
          m && (m.standard_template === "vllm" || m.standard_template === "tgi")
      );

    if (modelsToTest.length === 0) {
      alert(
        "No valid text generation models selected. Please select VLLM or TGI models."
      );
    ...
    latencyResults.innerHTML =
      '<div style="color: var(--color-text); margin-bottom: 1rem;">Testing in progress...</div>';
  ...
  runTokenTestBtn.addEventListener("click", async () => {
    ...
    // Clear previous results
    latencyResults.innerHTML =
      '<div style="color: var(--color-text); margin-bottom: 1rem;">Token testing in progress...</div>';
  ...
  function selectTokenFile(inputContext) {
    ...
    const suitableFiles = availableTokenFiles.filter(
      (f) => f.tokenCount < maxInputTokens
    );

    if (suitableFiles.length === 0) {
      // If no suitable file found, use the smallest one
      console.warn(
        `No token file smaller than ${maxInputTokens} (context: ${inputContext} - ${RESERVED_OUTPUT_TOKENS}), using smallest available`
      );
    ...
    console.log(
      `Selected ${selectedFile.filename} (${selectedFile.tokenCount} tokens) for context ${inputContext} (max input: ${maxInputTokens})`
    );
    return selectedFile;
  }
  ...
  async function testModelWithTokenFile(model) {
    ...
    try {
      ...
      console.log(
        `📄 Testing ${model.name} (context: ${inputContext}) with ${tokenFile.filename} (${tokenFile.tokenCount} tokens)`
      );
  ...
  function displayLatencyResults(results) {
    ...
    let html = `
            <div style="margin-bottom: 1rem; padding: 1rem; background: var(--color-surface); border-radius: 8px; border: 1px solid var(--color-border);">
                <h3 style="margin: 0 0 0.5rem 0; color: var(--color-text);">Test Results (${
                  results.length
                } models tested)</h3>
      ...
      html += `
                <div style="padding: 1rem; background: var(--color-surface); border-radius: 8px; border: 1px solid var(--color-border); margin-bottom: 1rem;">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 0.5rem; flex-wrap: wrap; gap: 0.5rem;">
                        <strong style="color: var(--color-text); flex: 1; min-width: 200px;">${
                          index + 1
                        }. ${result.name}</strong>
                    ...
                    ${
                      result.tokenFile
                        ...
                        <div style="color: var(--color-accent); font-size: 0.85rem; margin-top: 0.5rem; padding: 0.5rem; background: var(--color-bg); border-radius: 4px;">
                            📄 <strong>Token Test:</strong> Used ${
                              result.tokenFile
                            } | Model Context: ${formatNumber(
                            result.inputContext
                          )}
                        </div>
                    `
                    ...
                    ${
                      result.error
                        ? `<div style="color: var(--color-error); font-size: 0.9rem; margin-top: 0.5rem;">❌ Error: ${result.error}</div>`
                        : ""
                    }
                    ${
                      result.usage
                    ...
                    ${
                      result.response
                        ...
                        <div style="margin-top: 0.75rem; padding: 0.75rem; background: var(--color-bg); border-radius: 4px; border-left: 3px solid var(--color-accent);">
                            ...
                            <div style="color: var(--color-text); font-size: 0.9rem; line-height: 1.6; max-height: 300px; overflow-y: auto; white-space: pre-wrap; word-wrap: break-word;">${escapeHtml(
                              result.response
                            )}</div>
                        </div>
                    `
